import{_ as a,c as n,o as p,a2 as e}from"./chunks/framework.DzQ5xTzt.js";const k=JSON.parse('{"title":"Redis","description":"","frontmatter":{},"headers":[],"relativePath":"document/services/linux/Redis数据库操作.md","filePath":"document/services/linux/Redis数据库操作.md"}'),i={name:"document/services/linux/Redis数据库操作.md"};function l(t,s,c,d,h,r){return p(),n("div",null,s[0]||(s[0]=[e(`<h1 id="redis" tabindex="-1">Redis <a class="header-anchor" href="#redis" aria-label="Permalink to &quot;Redis&quot;">​</a></h1><p>Redis ：Remote Dictionary Server(远程字典服务器)，是完全开源免费的，用 C 语言编写的遵守 BSD 协议，是一个高性能的(key/value)分布式内存数据库，基于内存运行并支持持久化的 NoSQL 数据库，是当前最热门的 NoSQL 数据库之一，也被人们称为数据结构服务器，Redis 与其他 key - value 缓存产品有以下三个特点 ：</p><p>1、Redis 支持数据的持久化，可以将内存中的数据保存在磁盘中，重启的时候可以再次加载进行使用。</p><p>2、Redis 不仅仅支持简单的 key - value 类型的数据，同时还提供 list、set、zset、hash 等数据结构的存储。</p><p>3、Redis 支持数据的备份，即 master - slave 模式的数据备份。</p><h2 id="redis-基本介绍" tabindex="-1">Redis 基本介绍 <a class="header-anchor" href="#redis-基本介绍" aria-label="Permalink to &quot;Redis 基本介绍&quot;">​</a></h2><ul><li>单进程：单进程模型来处理客户端的请求。对读写等事件的相应。是通过 epoll 函数的包装来做到的。Redis 的实际处理速度完全依靠主进程的执行效率。Epoll 是 linux 内核为处理大批量文件描述符而作了改进的 epoll，是 linux下多路复用 IO 接口 select/poll 的增强版本，它能显著提高程序在大量并发连接中只有少量活跃的情况下的下同 CPU 利用率。</li><li>默认 16 个库，类似数组下标从零开始，初始默认使用零号库。</li><li>select 命令切换数据库。</li><li>Dbsize 查看当前数据库的 key 的数量。</li><li>Flushdb：清空当前库。</li><li>Flushall：通杀全部库。</li><li>统一密码管理，16个库都是同样密码，要么都ok要么一个也连接不上。</li><li>Redis索引都是从零开始。</li><li>为什么默认端口是6379。</li></ul><p>redis 的配置文件 redis-services-config.conf 中有这一行配置，说明 redis 是有 16 个仓库</p><p><code># Set the number of databases. The default database is DB 0, you can select</code></p><p><code># a different one on a per-connection basis using SELECT &lt;dbid&gt; where</code></p><p><code># dbid is a number between 0 and &#39;databases&#39;-1</code></p><p><code>databases 16</code></p><h2 id="redis-shell-操作" tabindex="-1">Redis Shell 操作 <a class="header-anchor" href="#redis-shell-操作" aria-label="Permalink to &quot;Redis Shell 操作&quot;">​</a></h2><h3 id="key-键操作" tabindex="-1">Key 键操作 <a class="header-anchor" href="#key-键操作" aria-label="Permalink to &quot;Key 键操作&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>del key                                    # 该命令用于在 key 存在时删除 key</span></span>
...
<span class="line"><span>type key                                   # 返回 key 所储存的值的类型</span></span></code></pre></div><h3 id="string-字符串类型" tabindex="-1">String 字符串类型 <a class="header-anchor" href="#string-字符串类型" aria-label="Permalink to &quot;String 字符串类型&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>set key value                      # 保存到库中</span></span>
...
<span class="line"><span>hincbyfloat key field value           # 对 Key 指定的 Hash 数据中的 field 的值进行计算，增加浮点数 value</span></span></code></pre></div><h3 id="set-无序集合类型" tabindex="-1">Set 无序集合类型 <a class="header-anchor" href="#set-无序集合类型" aria-label="Permalink to &quot;Set  无序集合类型&quot;">​</a></h3><p>Redis 的 Set 是 String 类型的无序集合。集合成员是唯一的，这就意味着集合中不能出现重复的数据。Redis 中集合是通过哈希表实现的，所以添加，删除，查找的复杂度都是 O(1) 。集合中最大的成员数为 232 - 1 (4294967295, 每个集合可存储40多亿个成员) 。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>redis 127.0.0.1:6379&gt; SADD runoobkey redis</span></span>
...
<span class="line"><span>在以上实例中我们通过 SADD 命令向名为 runoobkey 的集合插入的三个元素</span></span></code></pre></div><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>SADD key member1 [member2]                         # 向集合添加一个或多个成员</span></span>
...
<span class="line"><span>在以上实例中我们通过命令 ZADD 向 redis 的有序集合中添加了三个值并关联上分数。</span></span></code></pre></div><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>ZADD key score1 member1 [score2 member2]         # 向有序集合添加一个或多个成员，或者更新已存在成员的分数</span></span>
...
<span class="line"><span>ZSCAN key cursor [MATCH pattern] [COUNT count]   # 迭代有序集合中的元素（包括元素成员和元素分值）</span></span></code></pre></div><h3 id="hyperloglog" tabindex="-1">HyperLogLog <a class="header-anchor" href="#hyperloglog" aria-label="Permalink to &quot;HyperLogLog&quot;">​</a></h3><p>HyperLogLog 是用来做基数统计的算法，HyperLogLog 的优点是，在输入元素的数量或者体积非常非常大时，计算基数所需的空间总是固定 的、并且是很小的，在 Redis 里面，每个 HyperLogLog 键只需要花费 12 KB 内存，就可以计算接近 2^64 个不同元素的基 数。这和计算基数时，元素越多耗费内存就越多的集合形成鲜明对比。</p><p>但是因为 HyperLogLog 只会根据输入元素来计算基数，而不会储存输入元素本身，所以 HyperLogLog 不能像集合那样，返回输入的各个元素 。 什么是基数? 比如数据集 {1, 3, 5, 7, 5, 7, 8}， 那么这个数据集的基数集为 {1, 3, 5 ,7, 8}, 基数(不重复元素)为5。 基数估计就是在误差可接受的范围内，快速计算基数，以下实例演示了 HyperLogLog 的工作过程：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>redis 127.0.0.1:6379&gt; PFADD runoobkey &quot;redis&quot;</span></span>
...
<span class="line"><span>2) &quot;zouzhu&quot;</span></span></code></pre></div><p>从上面代码可以看到，先使用 multi 启动了 redis 的事务，因此进入了 set 和 get 命令，我们可以发现它并未马上执行，而是返回一个 “QUEUED” 的结果。这说明 redis 将其放入队列中，并不会马上执行，当命令执行到 exec 的时候它就会把队列中的命令发送给 redis 服务器，这样存储在队列中的命令就会被执行了，所以才会有 ok 和 value 的输出返回 。</p><h3 id="discard-取消事务" tabindex="-1">discard 取消事务 <a class="header-anchor" href="#discard-取消事务" aria-label="Permalink to &quot;discard 取消事务&quot;">​</a></h3><p>discard 命令后，在使用 exec 命令就会报错，因为 discard 命令已经取消了事务中的命令，而到了 exec 命令时，队列里面已经没有命令可以执行了，所以就出现了报错的情况 。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>127.0.0.1:6379&gt; multi</span></span>
<span class="line"><span>OK</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">// 2019-07-30 15:19:27.576  INFO 12512 --- [       Thread-1] o.s.s.concurrent.ThreadPoolTaskExecutor</span></span></code></pre></div><h3 id="发布订阅" tabindex="-1">发布订阅 <a class="header-anchor" href="#发布订阅" aria-label="Permalink to &quot;发布订阅&quot;">​</a></h3><p>Redis 发布订阅 (pub/sub) 是一种消息通信模式：发送者(pub)发送消息，订阅者(sub)接收消息 ，可以同时有几个订阅者，也可同时有几个发布者，现在我们演示怎么发布订阅 。 创建一个订阅对象，用于接收消息</p><p>创建一个发布对象，用于发布消息</p><p>发布订阅的相关命令</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">psubscrice  pattern [pattern ...]               # 订阅一个或多个符合给定模式的频道 .</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">unsubscribe [channel [channel ...]]             # 指退订给定的频道 .</span></span></code></pre></div><h2 id="redis-服务器操作" tabindex="-1">Redis 服务器操作 <a class="header-anchor" href="#redis-服务器操作" aria-label="Permalink to &quot;Redis 服务器操作&quot;">​</a></h2><h3 id="服务器管理" tabindex="-1">服务器管理 <a class="header-anchor" href="#服务器管理" aria-label="Permalink to &quot;服务器管理&quot;">​</a></h3><p>Redis 服务器命令主要是用于管理 redis 服务。以下实例演示了如何获取 redis 服务器的统计信息：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>redis 127.0.0.1:6379&gt; INFO</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>db3:keys=1,expires=0,avg_ttl=0</span></span></code></pre></div><h3 id="redis-服务器的相关命令" tabindex="-1">Redis 服务器的相关命令 <a class="header-anchor" href="#redis-服务器的相关命令" aria-label="Permalink to &quot;Redis 服务器的相关命令&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>异步执行一个 AOF（AppendOnly File） 文件重写操作</span></span>
...
<span class="line"><span>用于复制功能(replication)的内部命令</span></span></code></pre></div><h3 id="redis-连接" tabindex="-1">Redis 连接 <a class="header-anchor" href="#redis-连接" aria-label="Permalink to &quot;Redis 连接&quot;">​</a></h3><p>Redis 连接命令主要是用于连接 redis 服务，以下实例演示了客户端如何通过密码验证连接到 redis 服务，并检测服务是否在运行：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>redis 127.0.0.1:6379&gt; AUTH &quot;password&quot;</span></span>
<span class="line"><span>OK</span></span>
<span class="line"><span>redis 127.0.0.1:6379&gt; PING</span></span>
<span class="line"><span>PONG</span></span></code></pre></div><p>Redis 连接命令，下表列出了 redis 连接的基本命令：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>序号  命令及描述</span></span>
...
<span class="line"><span>切换到指定的数据库</span></span></code></pre></div><h3 id="redis-脚本" tabindex="-1">Redis 脚本 <a class="header-anchor" href="#redis-脚本" aria-label="Permalink to &quot;Redis 脚本&quot;">​</a></h3><p>Redis 脚本使用 lua 解释器来执行脚本。 Redis 2.6 版本通过内嵌支持 Lua 环境。执行脚本的常用命令为 eval 语法，eval 命令的基本语法如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>redis 127.0.0.1:6379&gt; eval lua-script key-number [key1 key2 key3 ...] [value1 value2 value3 ...]</span></span></code></pre></div><p>eval 代表执行 Lua 语言的命令，Lua-script 代表 Lua 语言脚本，key-num 整数代表参数中有多少个key，需要注意的是redis中key是从1开始的，如果没有key的参数，那么写 0，演示常用的写法 这个脚本只是返回一个字符串，并不需要任何参数，所以 key-num 填写了 0，代表着没有任何 key 参数 。按照脚本的结果就是返回 hello word</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>127.0.0.1:6379&gt; eval &quot;return &#39;hello redis lua ! &#39;&quot;  0</span></span>
<span class="line"><span>&quot;hello redis lua ! &quot;</span></span></code></pre></div><p>设置一个键值对，可以在 Lua 语言中采用 redis.call(command，key[param1] ，key[param2 .. ]) ，其中 command 是命令，包括 set，get，del 。Key 是被操作的键值对 。param1，param2 ...代表给 key 的参数</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>127.0.0.1:6379&gt; eval &quot;redis.call(&#39;set&#39;, KEYS[1], ARGV[1])&quot; 1 Email [zouzhu@net.cn.com](mailto:zouzhu@net.cn.com)</span></span>
<span class="line"><span>(nil)</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>&quot;[zouzhu@net.cn.com](mailto:zouzhu@net.cn.com)&quot;</span></span></code></pre></div><p>加密脚本，有时可能需要多次执行同样一段脚本，这个时候可以使用Redis缓存脚本的功能，在redis中脚本会通过 sha-1 签名算法加密脚本，可以通过这个脚本执行 Lua，$ script load</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>127.0.0.1:6379&gt; script load &quot;return redis.call(&#39;get&#39;, KEYS[1])&quot;</span></span>
...
<span class="line"><span>&quot;[zouzhu@net.cn.com](mailto:zouzhu@net.cn.com)&quot;</span></span></code></pre></div><p>以下实例演示了 redis 脚本工作过程：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>redis 127.0.0.1:6379&gt; eval &quot;return {KEYS[1],KEYS[2],ARGV[1],ARGV[2]}&quot; 2 key1 key2 first second</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>SCRIPT LOAD script</span></span></code></pre></div><h3 id="redis-配置-备份-持久化-主从配置" tabindex="-1">Redis 配置(备份，持久化，主从配置) <a class="header-anchor" href="#redis-配置-备份-持久化-主从配置" aria-label="Permalink to &quot;Redis 配置(备份，持久化，主从配置)&quot;">​</a></h3><p>Redis 备份( 持久化 ) redis 中存在两种方式的备份 一种是快照，它是备份当前瞬间 redis 在内存中的数据记录 ；对于快照备份而言，如果当前Redis的数据量大，备份可能导致 redis 卡顿，但是恢复重启要执行是比较快速的 ； 另一种是追加文件aof，其作用就是当 redis 执行写命令后，在一定的条件下将执行的写命令依次保存在 redis 的文件中，将来就可以依次执行那些保存的命令恢复 redis 的数据了 。对于aof 备份而言，它只是追加写入命令，所以备份一般不会造成 redis 卡顿，但是恢复重启要执行更多的命令，备份文件可能也很大 使用者使用的时候要注意，在redis 中允许使用其中的一种，同时使用两种，或两者都不用，所以具体使用何种方式进行备份和持久化是用户可以通过配置决定的</p><p>service-redis-config</p><p>当 900 秒执行 1 个写命令时 , 启动快照备份 ， 当 300 秒执行 10 个写命令时 , 启动快照备份</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>save 900 1</span></span>
<span class="line"><span>save 300 10</span></span>
<span class="line"><span>save 60 10000</span></span></code></pre></div><p>bgsave 它是一个异步保存命令，也就是系统将启动另外一条进程，对redis的数据保存到对应的数据文件中，它和 save命令最大的不同是它不会阻塞客户端的写入，也就是在执行bgsave的时候，允许客户端继续读/写 redis，在默认情况下，如果 redis执行bgsave失败后，redis 将停止接受写操作，这样是一种强硬的方式让用户知道数据不能正确的持久化到磁盘，否则就会没人注意到灾难的发生，如果后台保存进程重新启动工作了，redis 也将自动允许写操作，然而如果安装了靠谱的监控，可能不希望 redis 这样做，那么你可以将其改为 no</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>stop-writes-on-bgsave-error yes</span></span></code></pre></div><p>这个命令意思是否对 rbd 文件进行检验，如果是将 rbd 文件检验，从 dbfilename 的配置可以知道，rdb文件实际是 redis 持久化的数据文件</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>rdbchecksum yes</span></span></code></pre></div><p>它是数据文件，当采用快照模式备份(持久化)时，redis将使用它保存数据，将来可以使用它恢复数据</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>dbfilename dump.rdb</span></span></code></pre></div><p>如果 appendonly 配置为 no，则不启动 AOF 方式进行备份 ，如果 appendonly 配置为 yes，则以AOF 方式备份 redis 数据，那么此时 redis 会按照配置，在特定的时候执行追加命令，用以备份数据</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>appendonly  no</span></span></code></pre></div><p>这里定义追加的写入文件为 appendonly.aof，采用 aof 追加文件备份的时候命令都会写到这里</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>appendfilename &quot;appendonly.aof&quot;</span></span></code></pre></div><p>持久化文件在 redis 安装目录下，我们也可以更改其路径</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>dir ./</span></span></code></pre></div><p>aof 文件和Redis命令是同步频率的，假设配置为always,其含义为当Redis执行命令的时候，则同时同步到AOF文件，这样会使得Redis同步刷新AOF文件，造成缓慢。而采用evarysec则代表每秒同步一次命令到AOF文件。采用no的时候，则由客户端调用命令执行备份，Redis 本身不备份文件。对于采用always配置的时候，每次命令都会持久化，它的 好处在于安全，坏处在于每次都持久化性能较差。采用evarysec则每秒同步，安全性不如always,备份可能会丢失1秒以内的命令，但是隐患也不大，安全度尚可，性能可以得到保障。采用no,则性能有所保障，但是由于失去备份，所以安全性比较差。笔者建议采用默认配置everysee,这样在保证性能的同时，也在一定程度 上保证了安全性</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>appendfsync always</span></span>
<span class="line"><span></span></span>
<span class="line"><span>appendfsync everysec</span></span>
<span class="line"><span></span></span>
<span class="line"><span>appendfsync no</span></span></code></pre></div>`,102)]))}const u=a(i,[["render",l]]);export{k as __pageData,u as default};
